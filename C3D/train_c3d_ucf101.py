...
flags.DEFINE_integer('max_steps', 38055, 'Number of steps to run trainer.')
...
def run_training():
  ...
  with graph.as_default() as g:
    ...
    X_batch, y_batch = get_tfrecord(FLAGS.batch_size * gpu_num, isTrain=True)
    X_batch1 = tf.reshape(X_batch, [FLAGS.batch_size * gpu_num, c3d_model.NUM_FRAMES_PER_CLIP, c3d_model.CROP_SIZE,
                                    c3d_model.CROP_SIZE, 3])
    print(y_batch.shape)
    x_v, y_v = get_tfrecord(FLAGS.batch_size * gpu_num, isTrain=False)
    x_v1 = tf.reshape(x_v,
                      [FLAGS.batch_size * gpu_num, c3d_model.NUM_FRAMES_PER_CLIP, c3d_model.CROP_SIZE,
                       c3d_model.CROP_SIZE, 3])
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      train_images, train_labels=sess.run([X_batch1, y_batch])
      ...
      if (step) % 10 == 0 or (step + 1) == FLAGS.max_steps:
        ...
        summary, acc,loss1 = sess.run(
                        [merged, accuracy,loss],
                        feed_dict={images_placeholder: train_images,
                            labels_placeholder: train_labels
                            })
        print ("accuracy: " + "{:.5f}".format(acc))
        print("loss==",np.mean(loss1))
        ...
        val_images, val_labels = sess.run([x_v1, y_v])
        summary, acc,loss1 = sess.run(
                        [merged, accuracy,loss],
                        feed_dict={
